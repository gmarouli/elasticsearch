...
public class TsdbDataStreamWithSecurityIT extends ESRestTestCase {
     ...
     *   <li>Index a document 30 days in the past with the limited user — rejected, outside the 7-day window.</li>
     * </ol>
     */
    public void testTsdbBackfillWriteWindowEnforcedByLimitedUser() throws Exception {
        ...
        putTemplateRequest.setJsonEntity("""
            ...
            """);
        assertOK(adminClient().performRequest(putTemplateRequest));
        assertOK(adminClient().performRequest(new Request("PUT", "/_data_stream/" + TSDB_DATA_STREAM_NAME)));
        ...
        try (var writerClient = buildClient(tsdbWriterRestClientSettings(), getClusterHosts().toArray(new HttpHost[0]))) {
            ...
            assertThat(
                "expected no errors indexing 30d-ago document before retention: " + response30Before,
                response30Before.get("errors"),
                is(false)
            );
            ...
            putLifecycle.setJsonEntity("""
                { "data_retention": "7d" }
                """);
            assertOK(adminClient().performRequest(putLifecycle));
            ...
            assertThat(
                "expected no errors indexing 6d-ago document within retention window: " + response6d,
                response6d.get("errors"),
                is(false)
            );

            // 30 days ago is outside the 7-day write window — the item must fail.
            Map<String, Object> response30After = entityAsMap(writerClient.performRequest(bulkCreateRequest(thirtyDaysAgo)));
            assertThat(
                "expected errors indexing 30d-ago document outside retention window: " + response30After,
                response30After.get("errors"),
                equalTo(true)
            );
        }
    }
