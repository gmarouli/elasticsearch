...
/**
 * This service provides helper methods until we have "proper" infrastructure for the layers
 */
public class PocHelper {
    ...
    public void maybeCreateDownsampleLayer(
        ...
        String layerName = DataStream.getDefaultDownsampleLayerIndexName(dataStreamName, interval);
        if (project.dataStreams().containsKey(layerName)) {
            listener.onResponse(null);
            return;
        }
        SubscribableListener.<Void>newForked(l -> maybeCreateDownsampleTemplate(project, dataStreamName, interval, l))
            .<AcknowledgedResponse>andThen(
                l -> client.execute(
                    CreateDataStreamAction.INSTANCE,
                    new CreateDataStreamAction.Request(
            ...
            .<Void>andThen((l, response) -> {
                if (response.isAcknowledged() == false) {
                    l.onFailure(new RuntimeException("Failed to create downsample layer [" + layerName + "]"));
                } else {
                    l.onResponse(null);
                }
            })
    ...
    public void maybeCreateDownsampleTemplate(
        ...
        String templateName = dataStreamName + "-" + interval.toString() + "-downsample-template";
        if (project.templatesV2().containsKey(templateName)) {
            listener.onResponse(null);
            return;
        }
        DownsampleConfig downsampleConfig = new DownsampleConfig(interval);
        Index sourceIndex = project.dataStreams().get(dataStreamName).getWriteIndex();
        ...
        SubscribableListener.<GetMappingsResponse>newForked(
            ...
            .<String>andThen((l, response) -> createDownsampleIndexMapping(project, sourceIndexMetadata, downsampleConfig, response, l))
            .<AcknowledgedResponse>andThen(
                (l, mapping) -> createDownsamplingTemplate(templateName, dataStreamName, downsampleConfig, sourceIndexMetadata, mapping, l)
            )
            .<Void>andThen((l, response) -> {
                if (response.isAcknowledged() == false) {
    ...
    private void createDownsamplingTemplate(
        ...
        Settings.Builder builder = Settings.builder()
            ...
            .put(IndexMetadata.INDEX_DOWNSAMPLE_INTERVAL.getKey(), downsampleConfig.getInterval().toString())
        ...
        try {
            ...
            request.indexTemplate(
                ComposableIndexTemplate.builder()
                    .indexPatterns(List.of(DataStream.getDefaultDownsampleLayerIndexName(dataStreamName, downsampleConfig.getInterval())))
                    .template(
                        Template.builder()
                            .settings(builder)
    ...
    public void createDownsampleIndexMapping(
        ...
        try {
            ...
            validateDownsamplingInterval(mapperService, downsampleConfig);
