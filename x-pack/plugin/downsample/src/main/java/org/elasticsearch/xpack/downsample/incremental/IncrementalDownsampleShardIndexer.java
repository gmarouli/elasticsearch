...
class IncrementalDownsampleShardIndexer {
    ...
    private final DownsamplingProgressTracker.TimeRange timeRange;
    ...
    IncrementalDownsampleShardIndexer(
        final DownsamplingProgressTracker.TimeRange timeRange,
        final Client client,
    ...
    public void execute(
        ...
        logger.info("Downsampling time range [{}] on shard {} started", timeRange, indexShard.shardId());
        ...
        logger.info("Downsampling task [" + timeRange + "] on shard " + indexShard.shardId() + " completed");
        listener.onResponse(numIndexed.longValue());
    }

    private Query createQuery() throws IOException {
        return QueryBuilders.rangeQuery("@timestamp").gte(timeRange.start()).lt(timeRange.end()).toQuery(searchExecutionContext);
    }
